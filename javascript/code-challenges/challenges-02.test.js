...
const raisedToTheThird = (arr) => {
  arr.forEach(value => raise.push(Math.pow(value, 3)));
  return raise;
};
...
const addValue = [];
const addOne = (arr) => {
  arr.map(value => addValue.push(value + 1));
  return addValue;
};
...
const addQuestion = (arr) => {
  arr.map(questionmark => Qmark.push(questionmark + '?'));
  return Qmark;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    timesTwo.push(Math.pow(2, arr[i]));
  }
  return timesTwo;
};
...
const forEachTwoToThe = (arr) => {
  const foreach = [];
  arr.forEach(output => foreach.push(Math.pow(2, output)));
  return foreach;
};
...
const mapTwoToThe = (arr) => {
  return arr.map(map6 => Math.pow(2, map6));
};
